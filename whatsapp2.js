...
(async () => {
  let browser;
  try {
    if (!fs.existsSync("mensagem.json")) {
      console.error("Erro: Arquivo mensagem.json não encontrado!");
      return;
    }

    const contatos = await fs.readJson("mensagem.json");

    if (!contatos.length) {
      console.error("Erro: Nenhuma mensagem encontrada no mensagem.json!");
      return;
    }
